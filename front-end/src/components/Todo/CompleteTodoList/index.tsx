      ...
      <section className="flex flex-col">
        ...
        >
          ...
          { !isLoading && completeTodoList.length > 0 &&
            (
              <ReactDnD
                todoList={localView !== "modal" ? completeTodoList.slice(0, 5)
                : completeTodoList}
                localView={localView}
                typeList="complete"
              />
        ...
        { localView !== "modal" &&
          (
            <div className="w-full flex flex-col items-center gap-2">
              ...
              <Button 
                text={ completeTodoList?.length ? infoTodoFolder.eraseAll 
                       : infoTodoFolder.emptyList 
                     } 
                ariaLabel={ completeTodoList?.length ? infoTodoFolder.eraseAll 
                            : infoTodoFolder.emptyList 
                          } 
                title={ completeTodoList?.length ? infoTodoFolder.eraseAll 
                        : infoTodoFolder.emptyList 
                      } 
                type="button" 
                width="w-[80%]"
